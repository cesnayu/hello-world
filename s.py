...
# List ini dipakai untuk Tab 1 (Grid) DAN Tab 2 (Sumber Data Top 20 Volume)
GRID_TICKERS = [
...
def get_stock_volume_stats(tickers_list):
    # Terima list langsung, bukan string
    if not tickers_list: return None
    
    # Batch download 1 bulan
...
# TAB LIST UPDATED (SECTOR REMOVED)
...
# === TAB 1: GRID (UPDATED LIST 40+ SAHAM) ===
with tab_grid:
    st.write("Grid Overview (All Popular Stocks)")
    if 'page' not in st.session_state: st.session_state.page = 1
    items_per_page = 20
    c1, _ = st.columns([1, 5])
    curr_page = c1.number_input("Halaman", 1, math.ceil(len(GRID_TICKERS)/items_per_page), key="grid_page")
...
with tab_vol:
    ...
    with c1:
        # Text Area agar bisa input banyak
    ...
    if btn_custom and vol_input:
        ...
        if clean_list:
            with st.spinner("Menganalisa input..."):
                ...
                st.session_state['vol_result'] = df_vol_result # Simpan session
    
    elif btn_top20:
        with st.spinner("Scanning Top 20 Volume (dari 50+ saham populer)..."):
            # Gunakan GRID_TICKERS sebagai universe
    ...
    # Cek Session State untuk Persistence
    if 'vol_result' in st.session_state and not st.session_state['vol_result'].empty:
        ...
        if sort_col == "Volume":
            df_show = df_show.sort_values(by="Volume", ascending=False)
        elif sort_col == "Est. Value":
            df_show = df_show.sort_values(by="Est. Value", ascending=False)
        elif sort_col == "Vol vs Avg":
            df_show = df_show.sort_values(by="Vol vs Avg", ascending=False)

        # Styling Highlight
        def highlight_vol_spike(val):
            # Highlight jika volume > 1.5x rata-rata
        ...
        st.dataframe(
            df_show.style.format({
                ...
                "Est. Value": "Rp {:,.0f}",
                "Vol vs Avg": "{:.2f}x"
            }).applymap(highlight_vol_spike, subset=['Vol vs Avg']),
