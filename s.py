...
import json
import os
import pytz # Untuk Timezone Jakarta
...
# Variabel Default Text untuk Input (Agar Tab 8 dan 9 tidak error)
...
def get_stock_history_bulk(tickers, period="3mo", interval="1d"):
    ...
    try:
        # Auto adjust False agar harga real (bukan adj close)
...
def get_single_stock_detail(ticker, period):
    ...
    try:
        ...
        # Timezone Conversion ke Jakarta
...
# FUNGSI MINI CHART KOMPLEKS (DUAL AXIS + CANDLESTICK/LINE)
def create_mini_chart_complex(df, ticker, period_code):
    ...
    # Calculate Percentage Change from Start of Visible Range
    first_price = df['Close'].iloc[0]
    df['Pct'] = ((df['Close'] - first_price) / first_price) * 100
    
    # Initialize Dual Axis Figure
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    
    # --- PLOT 1: PRICE (LEFT Y) ---
    if period_code == "1d":
        # Candlestick untuk 1 Hari
        ...
        # Line Chart untuk selain 1 Hari
        # Warna Line berdasarkan trend terakhir
    ...
    # --- PLOT 2: PERCENTAGE (RIGHT Y - INVISIBLE TRACE FOR SCALING) ---
    # Trik: Plot garis transparan di sumbu kanan agar grid line sinkron
    # Sebenarnya di Plotly gridline kiri dan kanan jarang pas 100% kecuali diatur manual tick-nya
    # Tapi kita set range agar proporsional
    fig.add_trace(go.Scatter(
        x=df.index, y=df['Pct'], mode='lines',
        line=dict(color='rgba(0,0,0,0)'), showlegend=False, hoverinfo='skip'
    ), secondary_y=True)

    # --- FITUR TAMBAHAN: GARIS BANTU 1 BULAN (KHUSUS 3M) ---
    if period_code == "3mo":
    ...
    fig.update_layout(
        ...
        height=200,
        showlegend=False,
        hovermode="x unified" # Hover muncul di kedua axis
    )
    
    # Konfigurasi Sumbu Y
    # Left: Price
    fig.update_yaxes(title=None, showticklabels=True, tickfont=dict(size=8), secondary_y=False)
    # Right: Pct (Sync Zero?) - Agak susah sync zero sempurna tanpa logic kompleks
    # Kita biarkan auto-scale tapi format persen
    fig.update_yaxes(title=None, showticklabels=True, tickfont=dict(size=8, color='gray'), tickformat=".1f%", secondary_y=True)
    
    fig.update_xaxes(showticklabels=False) # Hilangkan jam/tanggal agar rapi di grid kecil
...
if menu == "📊 Grid Overview":
    ...
    if final_tickers:
        ...
        with st.spinner(f"Memuat grafik {period_label}..."):
            # Batch Download dengan interval dinamis
...
# === PAGE 2: BANDINGKAN ===
    ...
    if not picked:
        ...
        with st.spinner("Mengambil data detail..."):
            # Comparison view pakai default 6mo daily
...
# === PAGE 9: WIN/LOSS ===
    ...
    # MENGGUNAKAN DEFAULT VARIABLE YANG SUDAH DIDEFINISIKAN UNTUK MENGHINDARI ERROR
    win_input = st.text_area("Input Saham:", value=DEFAULT_INPUT_TXT, height=100, key="win_in")
