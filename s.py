...
# TAB 2 CHANGE: Default Volume Watchlist KOSONG (Hanya user input)
if 'vol_watchlist' not in st.session_state:
    st.session_state.vol_watchlist = saved_data["vol_watchlist"] if (saved_data and "vol_watchlist" in saved_data) else []

# --- 4. DATA MAPPING (Helper) ---
SECTOR_MAP = {
...
# === TAB 1: GRID VIEW (USER INPUT ONLY) ===
with tab_grid:
    st.write("Grid Overview (Input Manual)")
    
    # 1. User Input untuk Grid
    default_grid = "BBCA.JK, BBRI.JK, BMRI.JK, ASII.JK"
    grid_input = st.text_area("Input Daftar Saham untuk Grid (Pisahkan koma):", value=default_grid)
    
    # 2. Parsing Input
    grid_tickers = []
    if grid_input:
        for t in grid_input.split(','):
            item = t.strip().upper()
            if item:
                if len(item) == 4 and item.isalpha(): item += ".JK"
                grid_tickers.append(item)
    grid_tickers = list(set(grid_tickers))

    # 3. Pagination Logic berdasarkan Input User
    if 'page' not in st.session_state: st.session_state.page = 1
    items_per_page = 20
    
    if grid_tickers:
        c1, _ = st.columns([1, 5])
        total_pages = math.ceil(len(grid_tickers)/items_per_page)
        curr_page = c1.number_input("Halaman", 1, max(1, total_pages), key="grid_page")
        ...
        st.info("Masukkan kode saham di atas untuk melihat Grid.")

# === TAB 2: VOLUME (CLEAN SLATE / MANUAL ONLY) ===
with tab_vol:
    st.header("Analisis Volume (Manual Watchlist)")
    
    # Input Area
    c_in, c_add = st.columns([3, 1])
    with c_in: n_vol = st.text_input("Tambah Saham:", placeholder="ADRO", key="v_in").strip().upper()
    with c_add: 
        st.write(""); st.write("") 
        if st.button("➕ Tambah", key="b_av"): 
            if n_vol:
                if len(n_vol) == 4 and n_vol.isalpha(): n_vol += ".JK"
                if n_vol not in st.session_state.vol_watchlist: 
                    st.session_state.vol_watchlist.append(n_vol)
                    save_data()
                    st.rerun()

    # List Management (Multiselect)
    if st.session_state.vol_watchlist:
        if st.button("🗑️ Hapus Semua List", key="del_vol_all"):
             st.session_state.vol_watchlist = []
             save_data()
             st.rerun()
             
        ed_vol = st.multiselect("Daftar Pantauan Volume:", options=st.session_state.vol_watchlist, default=st.session_state.vol_watchlist, key="ms_v")
        if len(ed_vol) != len(st.session_state.vol_watchlist): 
            st.session_state.vol_watchlist = ed_vol
            save_data()
            st.rerun()
            
        with st.spinner("Load Data Volume..."):
        ...
        st.info("List Volume kosong. Tambahkan saham di atas.")
