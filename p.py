...
# --- KONFIGURASI HALAMAN ---
st.set_page_config(page_title="Tes Filter Volatilitas", layout="wide")

# --- LIST SAHAM CONTOH (Campuran Likuid & Volatil) ---
TEST_TICKERS = [
...
def get_data_for_volatility(tickers, period='1mo'):
    results = []
    
    # Download massal biar cepat
    tickers_str = " ".join(tickers)
    data = yf.download(tickers_str, period=period, group_by='ticker', threads=True, progress=False)
    
    for ticker in tickers:
        try:
            # Handling struktur data multi-index
            if len(tickers) > 1:
                if ticker not in data.columns.levels[0]: continue
                df = data[ticker].dropna()
            else:
                df = data.dropna() # Jika cuma 1 saham
            
            if len(df) < 5: continue
            
            # --- PERHITUNGAN METRIK ---
            
            # 1. Harga Terkini & Kemarin
            current_price = df['Close'].iloc[-1]
            prev_close = df['Close'].iloc[-2]
            
            # 2. Perubahan Hari Ini
            day_change = ((current_price - prev_close) / prev_close) * 100
            
            # 3. Perubahan 1 Minggu (5 Candle terakhir)
            if len(df) >= 6:
                week_close = df['Close'].iloc[-6]
                week_change = ((current_price - week_close) / week_close) * 100
            else:
                week_change = 0
            
            # 4. VOLATILITAS (RANGE) dalam Periode Ini
            # Rumus: Seberapa lebar jarak High Tertinggi vs Low Terendah dalam periode chart
            ...
            results.append({
                ...
                'Volatilitas Range (%)': volatility_range, # Ini untuk filter
                'Data': df,
                'Low_Period': period_low # Untuk normalisasi grafik
            })
...
def plot_volatility_chart(data, ticker, low_ref, volatility_val):
    """
    Grafik khusus dimana Sumbu Y adalah PERSENTASE kenaikan dari titik terendah.
    """
    # Normalisasi harga menjadi Persentase dari Low terendah
    # 0% = Titik terendah di periode chart
    pct_series = ((data['Close'] - low_ref) / low_ref) * 100
    ...
    fig.update_layout(
        title=dict(
            text=f"<b>{ticker}</b> (Range: {volatility_val:.2f}%)",
            font=dict(size=14),
            x=0,
            xanchor='left'
        ),
        margin=dict(l=10, r=40, t=40, b=20), # Margin kanan diperbesar untuk angka Y
        height=200,
        showlegend=False,
        # Sumbu Y di KANAN (Side Right)
        yaxis=dict(
            side='right',
            title=None,
            showgrid=True,
            gridcolor='lightgray',
            ticksuffix='%', # Tambah tanda % di angka
            zeroline=False
        ),
        # Sumbu X
        xaxis=dict(
...
st.title("🧪 Prototype Scanner Volatilitas")
st.markdown("Filter saham berdasarkan **seberapa lebar** pergerakannya (High-Low) dalam periode tertentu.")

# 1. CONTROLS
col_ctrl1, col_ctrl2 = st.columns(2)
with col_ctrl1:
    # Filter Slider
    min_vol = st.slider("🔍 Cari Saham dengan Range Volatilitas Minimal (%):", 0.0, 20.0, 5.0, step=0.5)
with col_ctrl2:
    period_select = st.selectbox("Periode Analisa:", ["1mo", "3mo", "5d"], index=0)

# 2. PROSES DATA
if st.button("Run Scanner"):
    with st.spinner("Menghitung volatilitas..."):
        df_scan = get_data_for_volatility(TEST_TICKERS, period=period_select)
    
    if not df_scan.empty:
        # FILTER DATA
        df_filtered = df_scan[df_scan['Volatilitas Range (%)'] >= min_vol]
        
        # URUTKAN (Sort) dari yang paling volatil
        df_filtered = df_filtered.sort_values(by='Volatilitas Range (%)', ascending=False)
        
        st.success(f"Ditemukan {len(df_filtered)} saham dengan volatilitas > {min_vol}%")
        ...
        for i, (index, row) in enumerate(df_filtered.iterrows()):
            with cols[i % 3]: # Loop kolom 0, 1, 2
                ...
                # Info Header Kecil
                st.markdown(f"""
                **{row['Ticker']}** | Rp {row['Harga']:,.0f}  
                1W: <span style='color:{'green' if row['1 Minggu (%)'] > 0 else 'red'}'>{row['1 Minggu (%)']:+.2f}%</span> | 
                Range: **{row['Volatilitas Range (%)']:.2f}%**
                """, unsafe_allow_html=True)
                
                # Grafik Khusus Sumbu Y Persen
                fig = plot_volatility_chart(row['Data'], row['Ticker'], row['Low_Period'], row['Volatilitas Range (%)'])
