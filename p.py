...
# --- KONFIGURASI HALAMAN ---
st.set_page_config(page_title="Dashboard Pagination", layout="wide")

# --- LIST SAHAM (Bisa ditambah sampai ratusan, loading tetap aman per page) ---
ALL_TICKERS = [
...
ITEMS_PER_PAGE = 12  # 3 Baris x 4 Kolom

# --- INISIALISASI SESSION STATE (Untuk simpan data agar tidak reload ulang) ---
if 'stock_cache' not in st.session_state:
    st.session_state['stock_cache'] = {} # Tempat nyimpen data saham
if 'current_view_mode' not in st.session_state:
    st.session_state['current_view_mode'] = "Intraday (1 Hari)"

# --- FUNGSI RESET CACHE (Dipanggil kalau ganti timeframe) ---
def reset_cache():
    st.session_state['stock_cache'] = {}

# --- FUNGSI CHART & DATA (Sama seperti sebelumnya) ---
...
def fetch_batch_data(tickers, mode):
    """Fungsi download data on-demand (hanya yang diminta)"""
    results = []
...
# --- UI DASHBOARD ---
st.title("⚡ Dashboard Saham (Mode Halaman)")

# 1. TOP BAR: Timeframe Selection
selected_mode = st.selectbox("⏱️ Timeframe:", ["Intraday (1 Hari)", "Short Term (5 Hari)", "History (1 Bulan)"])

# Cek apakah mode berubah. Jika berubah, reset cache agar download data baru.
if selected_mode != st.session_state['current_view_mode']:
    st.session_state['current_view_mode'] = selected_mode
    reset_cache() # Hapus data lama karena timeframenya beda
    st.rerun()
...
with col_page2:
    # Selector Halaman
    current_page = st.number_input("📄 Pindah Halaman", min_value=1, max_value=total_pages, value=1)

# Tentukan saham mana yang harus ditampilkan di halaman ini
start_idx = (current_page - 1) * ITEMS_PER_PAGE
end_idx = start_idx + ITEMS_PER_PAGE
page_tickers = ALL_TICKERS[start_idx:end_idx]

st.info(f"Menampilkan Saham {start_idx+1} - {min(end_idx, total_stocks)} dari {total_stocks}")

# 3. PROSES DATA PER HALAMAN
# Cek di session_state, saham mana di halaman ini yang BELUM punya data
tickers_to_fetch = [t for t in page_tickers if t not in st.session_state['stock_cache']]

if tickers_to_fetch:
    with st.spinner(f"Sedang mengambil data untuk {len(tickers_to_fetch)} saham baru..."):
        # Download hanya yang kurang
        new_data = fetch_batch_data(tickers_to_fetch, selected_mode)
        # Simpan ke cache session_state
        for item in new_data:
            st.session_state['stock_cache'][item['Ticker'] + ".JK"] = item

# 4. TAMPILKAN GRID
# Ambil data dari cache session_state untuk halaman ini
page_data_list = []
...
if page_data_list:
    df_display = pd.DataFrame(page_data_list)
    
    # Sorting Lokal (Hanya untuk halaman ini)
    col_s1, col_s2 = st.columns([2, 2])
    with col_s1:
        sort_opt = st.selectbox("Urutkan (Halaman Ini):", ["Default (Urut Abjad)", "Volatilitas Tertinggi", "Kenaikan Tertinggi"])
        
    if sort_opt == "Volatilitas Tertinggi":
        df_display = df_display.sort_values(by='Volatilitas', ascending=False)
    elif sort_opt == "Kenaikan Tertinggi":
    ...
    st.warning("Gagal mengambil data untuk halaman ini. Coba refresh atau cek koneksi.")
