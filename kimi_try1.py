...
st.set_page_config(layout="wide", page_title="Stock Weekly Dashboard")
...
def download_data(_tickers_tuple, start_date, end_date):
    """
    Download data dengan parameter tuple (hashable) alih-alih list
    """
    data = yf.download(
        list(_tickers_tuple),  # Convert kembali ke list untuk yfinance
        start=start_date,
        end=end_date,
        group_by="ticker",
        auto_adjust=False,
        progress=False,
        threads=False  # Disable threading untuk menghindari konflik di Streamlit Cloud
    )
    return data
...
def get_stock_data(tickers):
    ...
    if not final_df.empty:
        final_df = final_df.sort_values(by="Today (%)", ascending=False)
    return final_df
...
# ==============================
# 5. CHART EXPLORER (FITUR BARU)
# ==============================

st.subheader("📈 Stock Price Chart Explorer")
selected_stocks = st.multiselect(
    "Cari dan Tambahkan Nama Saham untuk melihat Grafik:",
    options=LIST_SAHAM,
    default=None,
    placeholder="Ketik kode saham... (contoh: BBCA.JK)"
)
...
if selected_stocks:
    ...
    if len(selected_stocks) == 1:
        ...
        plot_df = chart_data['Close']
    else:
        plot_df = chart_data['Close']

    st.line_chart(plot_df)
else:
    st.info("Gunakan kolom pencarian di atas untuk membandingkan pergerakan harga saham.")

st.caption(f"Data minggu dimulai: {start_of_week.strftime('%Y-%m-%d')} | Auto-refresh 5 menit.")
