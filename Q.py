...
st.set_page_config(page_title="Scanner Seluruh Saham", layout="wide")

# --- 1. DAFTAR SAHAM (Bisa diganti list manual 800 saham) ---
# Contoh: Kita ambil Top 100 saham likuid (Kompas100) biar demo ini jalan lancar.
# Kalau mau SEMUA, ganti list ini dengan list manual kamu.
DEFAULT_TICKERS = [
...
# Tambahkan sisanya jika ingin full IHSG...

# --- 2. FUNGSI PENGAMBIL DATA DENGAN BATCHING ---

@st.cache_data(ttl=600) # Cache 10 menit
def get_data_batch(tickers_list):
    """
    Mengambil data dalam potongan (chunk) kecil @50 saham
    agar tidak timeout/crash.
    """
    all_results = []
    chunk_size = 50 # Ambil 50 saham per putaran
    
    # Buat container untuk progress bar
    ...
    for i in range(0, total_tickers, chunk_size):
        ...
        # Update status
        percent = min((i + chunk_size) / total_tickers, 1.0)
        status_text.text(f"Scanning {i} - {i+len(batch_tickers)} dari {total_tickers} saham...")
        progress_bar.progress(percent)
        
        try:
            # Download batch ini
            data = yf.download(batch_str, period='5d', group_by='ticker', threads=True, progress=False)
            
            # Proses data batch ini
            for ticker in batch_tickers:
                try:
                    # Handle struktur data pandas yang berbeda (Single vs MultiIndex)
                    ...
                    if len(hist) > 1:
                        ...
                        # Hitung metrik trading
                        change = ((curr - prev) / prev) * 100
                        volatility = ((hist['High'].iloc[-1] - hist['Low'].iloc[-1]) / hist['Open'].iloc[-1]) * 100
                        volume_val = hist['Close'].iloc[-1] * hist['Volume'].iloc[-1]
                        
                        all_results.append({
                            'Ticker': t_name.replace('.JK', ''),
                            'Harga': curr,
                            'Change%': change,
                            'Volatility%': volatility,
                            'Value (Rp)': volume_val,
                            'Data': hist # Simpan history pendek
                        })
                except:
                    continue
        except:
...
st.title("🇮🇩 Scanner Pasar (Batch Mode)")
st.write("Mode ini memecah download menjadi bagian-bagian kecil agar server tidak crash saat scan ratusan saham.")

# Input manual jika user mau tambah saham
user_input = st.text_area("Tambah Saham Manual (pisahkan dengan koma)", "GOTO.JK, BBCA.JK")
if user_input:
    custom_list = [x.strip() for x in user_input.split(',')]
    # Gabung dengan default dan hapus duplikat
    FINAL_LIST = list(set(DEFAULT_TICKERS + custom_list))
else:
    FINAL_LIST = DEFAULT_TICKERS

if st.button("🚀 Mulai Scan"):
    df = get_data_batch(FINAL_LIST)
    
    if not df.empty:
        # FILTER OTOMATIS ALA TRADER
        # Hanya tampilkan yang Value Transaksinya > 1 Milyar & Volatilitas > 2%
        
        st.success(f"Berhasil scan {len(df)} saham.")
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.subheader("🔥 Paling Volatil (Enak buat Scalping)")
            # Filter: Volatilitas tinggi, Transaksi lumayan
            df_volatil = df[df['Value (Rp)'] > 5_000_000_000].nlargest(10, 'Volatility%')
            st.dataframe(
                df_volatil[['Ticker', 'Harga', 'Volatility%', 'Change%']],
                hide_index=True,
                column_config={
                    "Volatility%": st.column_config.ProgressColumn("Range Harian", format="%.2f%%", min_value=0, max_value=20),
                    "Change%": st.column_config.NumberColumn(format="%.2f%%")
                }
            )
            
        with col2:
            st.subheader("💰 Paling Likuid (Rame Transaksi)")
            df_liquid = df.nlargest(10, 'Value (Rp)')
            st.dataframe(
                df_liquid[['Ticker', 'Harga', 'Value (Rp)', 'Change%']],
                hide_index=True,
                column_config={
                    "Value (Rp)": st.column_config.NumberColumn(format="Rp %.0f"),
                    "Change%": st.column_config.NumberColumn(format="%.2f%%")
                }
            )
            
    else:
        st.error("Tidak ada data yang didapat.")
