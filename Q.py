...
# Daftar saham campuran (Bluechip + Second Liner yang sering volatil)
STOCK_LIST = [
...
def scan_stocks(tickers):
    print("Sedang memindai pasar... Mohon tunggu.")
    results = []
    
    for ticker in tickers:
        try:
            # Ambil data 5 hari terakhir (interval 1 hari)
            # Kita butuh data harian untuk melihat range High-Low
            ...
            # 1. Hitung Volatilitas Harian Rata-rata (Average True Range Concept)
            # Rumus sederhana: ((High - Low) / Open) * 100
            hist['Daily_Range_Pct'] = ((hist['High'] - hist['Low']) / hist['Open']) * 100
            avg_volatility = hist['Daily_Range_Pct'].mean()
            
            # 2. Hitung Likuiditas (Rata-rata Transaksi Harian dalam Rupiah)
            # Volume * Close Price (Estimasi)
            hist['Value'] = hist['Volume'] * hist['Close']
            avg_value = hist['Value'].mean()
            
            # Data Terakhir
            last_close = hist['Close'].iloc[-1]
            last_vol = hist['Daily_Range_Pct'].iloc[-1]
            
            results.append({
                'Ticker': ticker.replace('.JK', ''),
                'Harga': last_close,
                'Avg_Volatility_5D': avg_volatility,  # Rata-rata fluktuasi 5 hari (%)
                'Last_Volatility': last_vol,          # Fluktuasi hari terakhir (%)
                'Avg_Value_IDR': avg_value,           # Rata-rata nilai transaksi (Rupiah)
            })
...
# --- EKSEKUSI ---
df_result = scan_stocks(STOCK_LIST)

# --- FILTERING ---
# Kriteria Filter Ala Scalper/Forex Trader:
# 1. Likuiditas: Rata-rata transaksi harian > 10 Milyar Rupiah (agar mudah keluar masuk)
# 2. Volatilitas: Rata-rata pergerakan harian > 2.5% (agar ada 'daging' buat dicopet)

min_liquidity = 10_000_000_000  # 10 Milyar
min_volatility = 2.5            # 2.5 Persen

# Terapkan Filter
screened_stocks = df_result[
    (df_result['Avg_Value_IDR'] >= min_liquidity) & 
    (df_result['Avg_Volatility_5D'] >= min_volatility)
].sort_values(by='Avg_Volatility_5D', ascending=False)

# Formatting Tampilan
pd.options.display.float_format = '{:,.2f}'.format

print("\n" + "="*80)
print("HASIL SCANNER: SAHAM VOLATIL & LIKUID (COCOK UNTUK SCALPING)")
print("="*80)
print(f"Syarat: Transaksi > Rp {min_liquidity:,.0f} & Volatilitas > {min_volatility}%")
print("-" * 80)

if not screened_stocks.empty:
    # Rename kolom biar lebih enak dibaca
    tampilan = screened_stocks.rename(columns={
        'Avg_Volatility_5D': 'Rata2 Fluktuasi (%)',
        'Avg_Value_IDR': 'Rata2 Transaksi (Rp)',
        'Last_Volatility': 'Fluktuasi Terakhir (%)'
    })
    print(tampilan[['Ticker', 'Harga', 'Rata2 Fluktuasi (%)', 'Fluktuasi Terakhir (%)', 'Rata2 Transaksi (Rp)']].to_string(index=False))
else:
    print("Tidak ada saham yang memenuhi kriteria filter saat ini.")

print("\n" + "="*80)
print("TIPS TRADING:")
print("1. Saham di atas punya range lebar, hati-hati 'gebuk' (banting) harga.")
print("2. Cek Orderbook (Bid/Offer) sebelum masuk.")
print("3. Gunakan chart 5 menit atau 15 menit untuk entry.")
