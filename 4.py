...
# --- 2. SIDEBAR CONTROLS ---
st.sidebar.header("⚙️ Pengaturan Chart")
...
MA_WINDOW = st.sidebar.slider(
    ...
    value=20,
    step=5,
    help="Window untuk Simple Moving Average"
)

# Stocks per page
STOCKS_PER_PAGE = st.sidebar.selectbox(
    "📋 Saham per Halaman:",
...
CHART_HEIGHT = st.sidebar.slider(
    "📏 Tinggi Chart per Baris:",
    min_value=2,
    max_value=6,
    value=3,
    step=1,
    help="Ukuran tinggi chart dalam satuan unit"
)
...
st.sidebar.markdown("""
### 💡 Tips Penggunaan:
- **5 Hari**: Analisis ultra short-term
- **1 Bulan**: Trading jangka pendek
- **6 Bulan**: Analisis swing trading
- **1 Tahun**: Tren tahunan
- **3-5 Tahun**: Long-term investing
- **10 Tahun/Max**: Historical analysis

**MA (Moving Average):**
- MA 20: Short-term trend
- MA 50: Medium-term trend
- MA 100-200: Long-term trend
""")
...
with col2:
    st.metric("Total Saham", f"{total_stocks:,}", delta=None)
with col3:
...
st.info(f"📊 Menampilkan saham **{start_idx+1} - {min(end_idx, total_stocks)}** dari **{total_stocks}** | Periode: **{selected_period_label}** | MA: **{MA_WINDOW}** hari")
...
if current_batch:
    with st.spinner(f"⏳ Mengambil data {selected_period_label} untuk {len(current_batch)} saham..."):
        ...
        if not data.empty:
            ...
            with col1:
                st.success(f"✅ Berhasil: {success_count} saham")
            with col2:
                if fail_count > 0:
                    st.warning(f"⚠️ Gagal: {fail_count} saham")
            with col3:
                st.info(f"📊 Total: {len(current_batch)} saham")
...
with col1:
    if page > 1:
        if st.button("⬅️ Halaman Sebelumnya", use_container_width=True):
            st.session_state.page = page - 1
            st.rerun()

with col2:
    st.markdown(f"<h4 style='text-align: center;'>Halaman {page} dari {total_pages}</h4>", unsafe_allow_html=True)

with col3:
    if page < total_pages:
        if st.button("Halaman Selanjutnya ➡️", use_container_width=True):
            st.session_state.page = page + 1
            st.rerun()
...
<div style='text-align: center; color: gray; font-size: 12px;'>
    ...
    <p>Dibuat dengan Streamlit & yfinance</p>
</div>
